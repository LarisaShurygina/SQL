...
union 
select clients.login from clients;
...
select count(color) as 'Количество цветов', color as 'Цвет', mark from auto
where mark = 'BMW'
group by color
union all
select count(color) as 'Количество цветов', color as 'Цвет', mark from auto
...
-- 2. Вывести на экран марку авто(количество) и количество авто не этой марки. 20 - BMW, AUDI - 30, LADA - 15

select mark, count(mark),
	(select count(mark) from auto) - count(mark) as 'Сумма других марок'
from auto
group by mark;
...
-- такой марки больше 2, записи на должны дублироваться
...
-- Задание №4.
-- Вывести Марку и цвет автомобиля - имя и фамилию покупателя, для всех покупателей,
-- которые живут в городе с населением больше 1 млн человек.

select man.firstname, man.lastname, auto.mark from man
right join auto
on man.phonenum = auto.phonenum;

-- Для всех покупателей:

select man.firstname, man.lastname, man.citycode, auto.mark, auto.color from man
right join auto
on man.phonenum = auto.phonenum
union
select man.firstname, man.lastname, man.citycode, auto.mark, auto.color from man
...
-- Для покупателей в городе больше 1000000: ДОДЕЛАТЬ
select man.firstname, man.lastname, man.citycode, auto.mark, auto.color from man
right join auto
on man.phonenum = auto.phonenum
where man.citycode = 1 or 5
union
select man.firstname, man.lastname, man.citycode, auto.mark, auto.color from man
left join auto
on man.phonenum = auto.phonenum
