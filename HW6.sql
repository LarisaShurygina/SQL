DROP DATABASE IF EXISTS semimar_6;
CREATE DATABASE semimar_6;
USE semimar_6;
-- пользователи
...
DELIMITER //
CREATE PROCEDURE filter(search_id int)
Begin
select user_id from users_communities
where community_id IN(
	select community_id from users_communities
    where user_id=search_id)
    and user_id<>search_id
    UNION
    select p2.user_id from profiles as p1
    join profiles as p2
    ON p1.hometown=p2.hometown
    where p1.user_id= search_id AND p2.user_id != search_id;
End //
DELIMITER ;
CALL filter(10);
...
-- помощью которой можно переместить любого (одного) пользователя из таблицы users
--  в таблицу users old. (использование транзакции с выбором commit или rollback
-- - обязательно).
...
-- зависимости от текущего времени суток. С 6:00 до 12:00 функция должна возвращать фразу "Доброе утро", 
...
begin
declare our_result varchar(40);
SELECT CASE
    when current_time() >= '06:00:00' and current_time() < '11:59:59' then 'Доброе утро'
    when current_time() >= '12:00:00' and current_time() < '17:59:59' then 'Добрый день'
    when current_time() >= '18:00:00' and current_time() < '23:59:59' then 'Добрый вечер'
    when current_time() >= '00:00:00' and current_time() < '05:59:59' then 'Доброй ночи'
    else 'ошибка'
  END into our_result;
